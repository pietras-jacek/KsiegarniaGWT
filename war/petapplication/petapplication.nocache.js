...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "petapplication" because\n';
  ...
  $wnd.__gwt_activeModules['petapplication'] = {
    'moduleName' : 'petapplication',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/petapplication.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['petapplication'].superdevmode = true;

  var expectedSuffix = '/petapplication.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + petapplication + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:petapplication';
  var devModeSessionKey = '__gwtDevModeSession:petapplication';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/petapplication/petapplication.nocache.js';
  ...
  $wnd.__gwt_activeModules['petapplication'].canRedirect = true;
  $wnd.__gwt_activeModules['petapplication'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING petapplication';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/petapplication?user.agent=' + ua + '&_callback=' + callback;
